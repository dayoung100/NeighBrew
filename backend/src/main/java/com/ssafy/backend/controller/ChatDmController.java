...
public class ChatDmController {
    ...
    //dm 방 별로 메세지들을 가져온다.
    @PostMapping("/message/{user1Id}/{user2Id}")
    public ResponseEntity<?> getDmMessages(//RequestBody String requestUser,
        ...
        Long requestUser =  request.get("requestUser");
        log.info("request유저{} : {}와{}의 메세지 가져오기", requestUser, user1Id, user2Id);
        try{
            return user1Id.compareTo(user2Id) < 0
            ? ResponseEntity.ok(chatDmService.findDmMessagesByRoomId(Long.valueOf(requestUser), user1Id, user2Id))
            : ResponseEntity.ok(chatDmService.findDmMessagesByRoomId(Long.valueOf(requestUser), user2Id, user1Id));
        }catch(Exception e){

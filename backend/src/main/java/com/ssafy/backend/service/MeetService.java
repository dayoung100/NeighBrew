...
public class MeetService {
    ...
    private void notifyFollowersAboutMeetCreation(User hostUser, Meet createdMeet) {
        ...
        for (Follow follower : followers) {
            pushService.send(hostUser, follower.getFollower(), PushType.MEETCREATED, hostUser.getNickname() + "님께서 회원님께서 모임(" + createdMeet.getMeetName() + ")을 생성했습니다.", "https://i9b310.p.ssafy.io/meet/" + createdMeet.getMeetId());
        }
    }
    ...
    public void applyMeet(Long userId, Long meetId) {
        ...
        pushService.send(attendUser, host, PushType.MEETACCESS, attendUser.getNickname() + "님께서 " + meetUserDto.getMeetRequestDto().getMeetName() + "모임에 참여하고 싶어 합니다.", "https://i9b310.p.ssafy.io/meet/" + attendMeet.getMeetId() + "/manage");
    }
    ...
    public String manageMeet(Long userId, Long meetId, boolean applyResult) {
        ...
        if (applyResult) {//신청 결과가 true
            ...
            pushService.send(host, manageUser, PushType.MEETACCESS, "회원님께서 모임(" + managementMeet.getMeetName() + ")참여 되셨습니다.\n 즐거운 시간 되세요.", "https://i9b310.p.ssafy.io/meet/" + managementMeet.getMeetId());

            return userId + "유저 " + meetId + "모임 신청 승인";
        } else {//신청 결과가 false
            meetUserService.deleteExitUser(userId, meetId, Status.APPLY);
            pushService.send(host, manageUser, PushType.MEETREJECT, "회원님께서 모임(" + managementMeet.getMeetName() + ")참여에 거절당했습니다.", "https://i9b310.p.ssafy.io/meet");

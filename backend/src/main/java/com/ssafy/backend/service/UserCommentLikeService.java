...
public class UserCommentLikeService {
    ...
    public boolean toggleUserLike(Long userId, Long reviewId) {
        ...
        if (existingLike.isPresent()) {
            ...
            drinkReviewRepository.save(drinkReview);
            pushService.send(user, drinkReview.getUser(), PushType.REVIEWLIKE, user.getName() + "님이 회원님의 후기를 좋아합니다. ", "https://i9b310.p.ssafy.io/drinkpost/" + drinkReview.getDrink().getDrinkId() + "/" + reviewId);
            return true;
        }
    }
